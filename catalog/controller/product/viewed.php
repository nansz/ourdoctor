...
class ControllerProductViewed extends Controller { 	
	public function index() { 
    	$this->language->load('product/viewed');
        ...
        if (isset($this->request->get['route']) && $this->request->get['route'] == 'product/product') {
            ...
            setcookie('viewed', implode(',',$viewed_products), time() + 60 * 60 * 24 * 1, '/', $this->request->server['HTTP_HOST']);
		...
		//var_dump($this->session->data['viewed']);
		...
		foreach ($products as $product_id) {
			...
			if ($product_info) {
				if ($product_info['image']) {
					$image = $this->model_tool_image->resize($product_info['image'], 149, 207);
				} else {
					$image = false;
				}
		...
		/*	
		$product_total = $this->model_catalog_product->getTotalProductViewed($data);
			
		$results = $this->model_catalog_product->getProductViewed($data);
		...
		$this->data['sorts'][] = array(
			...
			'href'  => $this->url->link('product/viewed', 'sort=p.sort_order&order=ASC' . $url)
		);
		...
		$this->data['sorts'][] = array(
			...
			'value' => 'ps.price-ASC',
			'href'  => $this->url->link('product/viewed', 'sort=ps.price&order=ASC' . $url)
		); 

		$this->data['sorts'][] = array(
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'ps.price-DESC',
			'href'  => $this->url->link('product/viewed', 'sort=ps.price&order=DESC' . $url)
		); 
		...
		if ($this->config->get('config_review_status')) {	
			...
			$this->data['sorts'][] = array(
				...
				'value' => 'rating-ASC',
				'href'  => $this->url->link('product/viewed', 'sort=rating&order=ASC' . $url)
			);
		}
		...
		$this->data['sorts'][] = array(
				...
				'value' => 'p.model-ASC',
				'href'  => $this->url->link('product/viewed', 'sort=p.model&order=ASC' . $url)
		); 

		$this->data['sorts'][] = array(
			'text'  => $this->language->get('text_model_desc'),
			'value' => 'p.model-DESC',
			'href'  => $this->url->link('product/viewed', 'sort=p.model&order=DESC' . $url)
		);
		...
		*/				
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/viewed.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/product/viewed.tpl';
		} else {
			$this->template = 'default/template/product/viewed.tpl';
		}
