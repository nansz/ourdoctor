...
class ControllerAccountOrder extends Controller {
	...
	public function index() {
        ...
        /** если мы вышли то мы вышли с аккаунта мы очищаем и выходим у учетной записи */
        if ($this->customer->isLogged()) {

            $Medical_Record_Info = $this->model_account_order->getMedicalRecord($this->customer->isLogged());
            $medical_record_id=$Medical_Record_Info[0]['medical_record_id'];
            $Medical_Record_Info = $this->model_account_order->getCaseHistory($medical_record_id);
            $this->data['record'] = array();

            $this->load->model('catalog/tests');

            foreach ($Medical_Record_Info as $result) {

                $total= $this->model_catalog_tests->getTotalCase_history();

                $count= $this->model_catalog_tests->getCountCase_history($result['disease_id']);
                $prosent= ($total/100)* $count;
                $this->data['record'][] = array(
                    'case_history_id'   => $result['case_history_id'],
