...
class ControllerInformationmedicinenews extends Controller
{

    public function index()
    {

        $this->language->load('information/medicinenews');
        ...
        if (isset($this->request->get['medicinenews_id'])) {
            $medicinenews_id = $this->request->get['medicinenews_id'];
        } else {
            $medicinenews_id = 0;
        }
        ...
        $articles_info = $this->model_catalog_medicinenews->getArticlesStory($medicinenews_id);
        ...
        if ($articles_info) {
            ...
            $this->data['breadcrumbs'][] = array(
                'href' => $this->url->link('information/medicinenews'),
                'text' => $this->language->get('heading_title'),
                'separator' => $this->language->get('text_separator')
            );

            $this->data['breadcrumbs'][] = array(
                'href' => $this->url->link('information/medicinenews', 'medicinenews_id=' . $this->request->get['medicinenews_id']),
            ...
            $results = $this->model_catalog_medicinenews->getProductImages($this->request->get['medicinenews_id']);
            ...
            $this->load->model('catalog/medicinenews');
            $staf_results = $this->model_catalog_medicinenews->getStaffStory($articles_info['author_id']);
...
//            $this->url->link('information/staff', 'staff_id=' . $articles_info['author_id']);
            $results = $this->model_catalog_medicinenews->getArticlesRelated($this->request->get['medicinenews_id']);
//
            $i=0;
            foreach ($results as $result) {
                if($i<2){
                foreach ($result as $resultz) {
                    ...
                    $this->data['article_related'][] = array(
                        'medicinenews_id' => $resultz['medicinenews_id'],
                        'thumb' => $image,
                        'name' => $resultz['title'],
...
//                        'author_href' => $this->url->link('information/staff', 'author_id=' . $resultz['author_id']),
                        ...
                        'href' => $this->url->link('information/medicinenews', 'medicinenews_id=' . $resultz['medicinenews_id'])
                    );
                }
                $i++;
            }
            ...
            $this->document->addLink($this->url->link('information/medicinenews', 'medicinenews_id=' . $this->request->get['medicinenews_id']), 'canonical');
...
//            $this->config->get('articles_popup_width'), $this->config->get('articles_popup_height'));
            ...
            $this->data['articles'] = $this->url->link('information/medicinenews');
            ...
            if ($this->data['referred'] != $this->data['refreshed']) {
                $this->model_catalog_medicinenews->updateViewed($this->request->get['medicinenews_id']);
            }

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/medicinenews.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/information/medicinenews.tpl';
            } else {
                $this->template = 'default/template/information/medicinenews.tpl';
            }
            ...
            if ($this->session->data['language'] == 'ru') {
                $articles_data = $this->model_catalog_medicinenews->getArticlesRus($data);
                $articles_total = $this->model_catalog_medicinenews->getTotalArticlesRus();
            } elseif ($this->session->data['language'] == 'ua') {
                $articles_data = $this->model_catalog_medicinenews->getArticlesUkr($data);
                $articles_total = $this->model_catalog_medicinenews->getTotalArticlesUkr();
            } elseif ($this->session->data['language'] == 'en') {
                $articles_data = $this->model_catalog_medicinenews->getArticlesEng($data);
                $articles_total = $this->model_catalog_medicinenews->getTotalArticlesEng();
            }


            if ($articles_data) {

                $this->document->setTitle(translatereturn("Новости медицины в Украине и мире  | Современные достижения медицинских технологий", "Новини медицини в Україні та світі | Сучасні досягнення медичних технологій", ""));
                $this->document->setDescription(translatereturn("Новости медицины — последние новости и любопытные статьи в сфере медицины и здравоохранения", "Новини медицини - останні новини та цікаві статті в сфері медицини та охорони здоров'я", ""));
                ...
                $pagination->url = $this->url->link('information/medicinenews', '&page={page}');
