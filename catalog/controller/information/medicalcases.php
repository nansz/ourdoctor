...
class ControllerInformationmedicalcases extends Controller
{

    public function index()
    {

        $this->language->load('information/medicalcases');
        ...
        if (isset($this->request->get['medicalcases_id'])) {
            $medicalcases_id = $this->request->get['medicalcases_id'];
        } else {
            $medicalcases_id = 0;
        }
        ...
        $articles_info = $this->model_catalog_medicalcases->getArticlesStory($medicalcases_id);
        ...
        if ($articles_info) {
            ...
            $this->data['breadcrumbs'][] = array(
                'href' => $this->url->link('information/medicalcases'),
                'text' => $this->language->get('heading_title'),
                'separator' => $this->language->get('text_separator')
            );

            $this->data['breadcrumbs'][] = array(
                'href' => $this->url->link('information/medicalcases', 'medicalcases_id=' . $this->request->get['medicalcases_id']),
            ...
            $results = $this->model_catalog_medicalcases->getProductImages($this->request->get['medicalcases_id']);
            ...
            $this->load->model('catalog/medicalcases');
            $staf_results = $this->model_catalog_medicalcases->getStaffStory($articles_info['author_id']);
...
//            $this->url->link('information/staff', 'staff_id=' . $articles_info['author_id']);
            $results = $this->model_catalog_medicalcases->getArticlesRelated($this->request->get['medicalcases_id']);
//
            $i=0;
            foreach ($results as $result) {
                if($i<2){
                foreach ($result as $resultz) {
                    ...
                    $this->data['article_related'][] = array(
                        'medicalcases_id' => $resultz['medicalcases_id'],
                        'thumb' => $image,
                        'name' => $resultz['title'],
...
//                        'author_href' => $this->url->link('information/staff', 'author_id=' . $resultz['author_id']),
                        ...
                        'href' => $this->url->link('information/medicalcases', 'medicalcases_id=' . $resultz['medicalcases_id'])
                    );
                }
                $i++;
            }
            ...
            $this->document->addLink($this->url->link('information/medicalcases', 'medicalcases_id=' . $this->request->get['medicalcases_id']), 'canonical');
...
//            $this->config->get('articles_popup_width'), $this->config->get('articles_popup_height'));
            ...
            $this->data['articles'] = $this->url->link('information/medicalcases');
            ...
            if ($this->data['referred'] != $this->data['refreshed']) {
                $this->model_catalog_medicalcases->updateViewed($this->request->get['medicalcases_id']);
            }

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/medicalcases.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/information/medicalcases.tpl';
            } else {
                $this->template = 'default/template/information/medicalcases.tpl';
            }
            ...
            if ($this->session->data['language'] == 'ru') {
                $articles_data = $this->model_catalog_medicalcases->getArticlesRus($data);
                $articles_total = $this->model_catalog_medicalcases->getTotalArticlesRus();
            } elseif ($this->session->data['language'] == 'ua') {
                $articles_data = $this->model_catalog_medicalcases->getArticlesUkr($data);
                $articles_total = $this->model_catalog_medicalcases->getTotalArticlesUkr();
            } elseif ($this->session->data['language'] == 'en') {
                $articles_data = $this->model_catalog_medicalcases->getArticlesEng($data);
                $articles_total = $this->model_catalog_medicalcases->getTotalArticlesEng();
            }


            if ($articles_data) {

                $this->document->setTitle(translatereturn("Медицинские кейсы | МДЦ-LUX", "Медичні кейси | МДЦ-LUX", ""));
                $this->document->setDescription(translatereturn("Медицинские кейсы ведущих мировых клиник собраны на нашем сайте", "Медичні кейси провідних світових клінік зібрані на нашому сайті", ""));
                ...
                $pagination->url = $this->url->link('information/medicalcases', '&page={page}');
