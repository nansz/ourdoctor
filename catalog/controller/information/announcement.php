...
class ControllerInformationannouncement extends Controller
{
    ...
    public function index()
    {
        ...
        $articles_info = $this->model_catalog_announcement->getArticlesStory($announcement_id);
        ...
        if ($articles_info) {
            ...
            if ($this->session->data['language'] == 'ua' && $articles_info['ukr'] == '0') {
                ...
                header("Location: /ua/announcement/");
                exit();
            }
            if ($this->session->data['language'] == 'en' && $articles_info['eng'] == '0') {
                header("HTTP/1.1 301 Moved Permanently");
                header("Location: /en/announcement/");
                exit();
            }
            ...
            $this->data['breadcrumbs'][] = array(
                'href' => $this->url->link('information/announcement', 'announcement_id=' . $this->request->get['announcement_id']),
            ...
            $results = $this->model_catalog_announcement->getProductImages($this->request->get['announcement_id']);
            ...
            $this->load->model('catalog/announcement');
            $staf_results = $this->model_catalog_announcement->getStaffStory($articles_info['author_id']);
...
//            $this->url->link('information/staff', 'staff_id=' . $articles_info['author_id']);
            $results = $this->model_catalog_announcement->getArticlesRelated($this->request->get['announcement_id']);
//
            $i=0;
            foreach ($results as $result) {
                if($i<2){
                    foreach ($result as $resultz) {
                        ...
                        $this->data['article_related'][] = array(
                            'announcement_id' => $resultz['announcement_id'],
                            'thumb' => $image,
                            'name' => $resultz['title'],
...
//                        'description' => html_entity_decode($resultz['description']),
                            ...
                            'href' => $this->url->link('information/announcement', 'announcement_id=' . $resultz['announcement_id'])
                        );
                    }
                    $i++;
                }
            ...
            $this->document->addLink($this->url->link('information/announcement', 'announcement_id=' . $this->request->get['announcement_id']), 'canonical');
...
//            $this->config->get('articles_popup_width'), $this->config->get('articles_popup_height'));
            ...
            $this->data['articles'] = $this->url->link('information/announcement');
            ...
            if ($this->data['referred'] != $this->data['refreshed']) {
                $this->model_catalog_announcement->updateViewed($this->request->get['announcement_id']);
            }

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/announcement.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/information/announcement.tpl';
            } else {
                $this->template = 'default/template/information/announcement.tpl';
            }
            ...
            if ($this->session->data['language'] == 'ru') {
                $articles_data = $this->model_catalog_announcement->getArticlesRus($data);
                $articles_total = $this->model_catalog_announcement->getTotalArticlesRus();
            } elseif ($this->session->data['language'] == 'ua') {
                $articles_data = $this->model_catalog_announcement->getArticlesUkr($data);
                $articles_total = $this->model_catalog_announcement->getTotalArticlesUkr();
            } elseif ($this->session->data['language'] == 'en') {
                $articles_data = $this->model_catalog_announcement->getArticlesEng($data);
                $articles_total = $this->model_catalog_announcement->getTotalArticlesEng();
            }
            ...
            if ($articles_data) {
                ...
                foreach ($articles_data as $result) {
                    $this->data['articles_data'][] = array(
                        ...
                        'href' => $this->url->link('information/announcement', 'announcement_id=' . $result['announcement_id']),
                ...
                $pagination->url = $this->url->link('information/announcement', '&page={page}');
