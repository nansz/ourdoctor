...
class Pagination {
	...
	public $text_first = '|&lt;';
	public $text_last = '&gt;|';
	public $text_next = '<img src="/img/next-pagination.svg" alt="" class="icon">';
	public $text_prev = '<img src="/img/prev-pagination.svg" alt="" class="icon"></a>';
	...
	public function render() {
		...
		if ($page >= 1) {
			...
			if($page == 1) {
				if($num_pages>1) {
					$output .= '<li><a>' . $this->text_prev . '</a></li> ';
				}
			}
			else if ($page == 2){
		...
		return ($output ? '<ul class="center">' . $output . '</ul>' : '') . '<div class="' . $this->style_results . '">' . str_replace($find, $replace, $this->text) . '</div>';
	}
}
?>
