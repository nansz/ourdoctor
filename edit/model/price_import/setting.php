...
class ModelPriceImportSetting extends Model {
    ...
    public function update_price(array $config)
    {
		...
		$this->db->query(sprintf('UPDATE %s SET `price_new`=`price`*%f', $this->tmp_tbl_name, $res['value']));

		if ($config['discount'] !== '') {
			$this->db->query(sprintf('UPDATE %s SET `price_new`=`price_new`*%f', $this->tmp_tbl_name, $config['discount']));
		}

		if ($config['margin'] !== '') {
			$this->db->query(sprintf('UPDATE %s SET `price_new`=`price_new`*%f', $this->tmp_tbl_name, $config['margin']));
		}
    }
