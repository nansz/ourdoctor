					...
					success: function(json) {
						...
						if (json) {
							for (i = 0; i < json.length; i++) {
								...
								var realFileName = json[i]['file'].split('.').slice(0, -1);
								var fileExtension = json[i]['filename'].split('.').pop();
								
								html += '<a file="' + json[i]['file'] + '"><img src="../image/cache/data/' + realFileName + '-100x100.' + fileExtension + '" title="' + json[i]['filename'] + '" /><br />' + name + '</a>';
							}
						}
