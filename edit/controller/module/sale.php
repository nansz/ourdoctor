...
class ControllerModuleSale extends Controller {
	...
	public function index() {
		...
		$this->model_catalog_sale->checkSale();
	...
	public function insert() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			$this->model_catalog_sale->addSale($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('module/sale/listing', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function update() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			$this->model_catalog_sale->editSale($this->request->get['sale_id'], $this->request->post);
	...
	private function getModule() { 
		...
		$this->data['entry_salepage_thumb'] = $this->language->get('entry_salepage_thumb');
		$this->data['entry_salepage_popup'] = $this->language->get('entry_salepage_popup');
		$this->data['entry_salepage_addthis'] = $this->language->get('entry_salepage_addthis');
		...
		if (isset($this->error['salepage_thumb'])) {
			$this->data['error_salepage_thumb'] = $this->error['salepage_thumb'];
		} else {
			$this->data['error_salepage_thumb'] = '';
		}
	
		if (isset($this->error['salepage_popup'])) {
			$this->data['error_salepage_popup'] = $this->error['salepage_popup'];
		} else {
			$this->data['error_salepage_popup'] = '';
		}
		...
		$this->data['sale'] = $this->url->link('module/sale/listing', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post[$this->_name . '_salepage_addthis'])) {
			$this->data[$this->_name . '_salepage_addthis'] = $this->request->post[$this->_name . '_salepage_addthis'];
		} else {
			$this->data[$this->_name . '_salepage_addthis'] = $this->config->get($this->_name . '_salepage_addthis');
		}
	...
	private function getList() {
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('module/sale/listing', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
		);
	
		$this->data['module'] = $this->url->link('module/sale', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['insert'] = $this->url->link('module/sale/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('module/sale/delete', 'token=' . $this->session->data['token'], 'SSL');
	
		$this->data['totalsale'] = $this->model_catalog_sale->getTotalSale();
	
		$this->load->model('tool/image');
	
		$this->data['sale'] = array();
	
		$results = $this->model_catalog_sale->getSale();
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('module/sale/update', 'token=' . $this->session->data['token'] . '&sale_id=' . $result['sale_id'], 'SSL')
			);
			...
			$this->data['sale'][] = array(
				...
				'selected'    	=> isset($this->request->post['selected']) && in_array($result['sale_id'], $this->request->post['selected']),
				'action'      		=> $action
			);
		}
	
		$this->template = 'module/sale/list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm() { 
		...
		if (!isset($this->request->get['sale_id'])) {
			$this->data['action'] = $this->url->link('module/sale/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('module/sale/update', 'token=' . $this->session->data['token'] . '&sale_id=' . $this->request->get['sale_id'], 'SSL');
		}
	
		$this->data['cancel'] = $this->url->link('module/sale/listing', 'token=' . $this->session->data['token'], 'SSL');
	
		if ((isset($this->request->get['sale_id'])) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$sale_info = $this->model_catalog_sale->getSaleStory($this->request->get['sale_id']);
		}
		$this->load->model('localisation/language');
	
		$this->data['languages'] = $this->model_localisation_language->getLanguages();
	
		if (isset($this->request->post['sale_description'])) {
			$this->data['sale_description'] = $this->request->post['sale_description'];
		} elseif (isset($this->request->get['sale_id'])) {
			$this->data['sale_description'] = $this->model_catalog_sale->getSaleDescriptions($this->request->get['sale_id']);
		} else {
			$this->data['sale_description'] = array();
		}
		...
		if (isset($this->request->post['meta_keyword'])) {
			...
			$this->data['meta_keyword'] = $this->model_catalog_sale->getSaleDescriptions($this->request->get['sale_id']);
		} else {
		...
		} elseif (isset($sale_info['date_added'])) {
		...
		if (isset($this->request->post['sale_store'])) {
			$this->data['sale_store'] = $this->request->post['sale_store'];
		} elseif (isset($sale_info)) {
			$this->data['sale_store'] = $this->model_catalog_sale->getSaleStores($this->request->get['sale_id']);
		} else {
			$this->data['sale_store'] = array(0);
		}
		...
		elseif (!empty($sale_info['keyword']))
		{
			$this->data['keyword'] = utf8_strtolower($sale_info['keyword']);
		}
		else
		{
			if (!empty($this->data['sale_description'][1]['title']))
			{
				$this->data['keyword'] = utf8_strtolower(translitIt($this->data['sale_description'][1]['title']));
			}
			else
			{
				$this->data['keyword'] = '';
		...
		if (isset($this->request->post['status'])) {
			...
			$this->data['status'] = $sale_info['status'];
		} else {
			$this->data['status'] = '';
		}
		...
		if (isset($this->request->post['image'])) {
			...
			$this->data['image'] = $sale_info['image'];
		} else {
			$this->data['image'] = '';
		}
		...
		} elseif (!empty($sale_info) && $sale_info['image'] && file_exists(DIR_IMAGE . $sale_info['image'])) {
			$this->data['thumb'] = $this->model_tool_image->resize($sale_info['image'], 100, 100);
		} else {
			$this->data['thumb'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}
	
		$this->template = 'module/sale/form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		...
		if (!$this->request->post['sale_headline_chars']) {
			$this->error['numchars'] = $this->language->get('error_numchars');
		}
	
		if (!$this->request->post['sale_thumb_width'] || !$this->request->post['sale_thumb_height']) {
			$this->error['salepage_thumb'] = $this->language->get('error_salepage_thumb');
		}
	
		if (!$this->request->post['sale_popup_width'] || !$this->request->post['sale_popup_height']) {
			$this->error['salepage_popup'] = $this->language->get('error_salepage_popup');
		}
	...
	private function validateForm() {
		...
		foreach ($this->request->post['sale_description'] as $language_id => $value) {
	...
	public function install() { 
		//create sale table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "sale`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "sale` (`sale_id` int(11) NOT NULL auto_increment, `status` int(1) NOT NULL default '0', `image` VARCHAR(255) COLLATE utf8_general_ci default NULL, `image_size` int(1) NOT NULL default '0', `date_added` date default NULL, `viewed` int(5) NOT NULL DEFAULT '0', PRIMARY KEY (`sale_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
		//create sale description table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "sale_description`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "sale_description` (`sale_id` int(11) NOT NULL default '0', `language_id` int(11) NOT NULL default '0', `title` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `meta_description` VARCHAR(255) COLLATE utf8_general_ci NOT NULL, `description` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `meta_keyword` varchar(255) COLLATE utf8_general_ci NOT NULL, PRIMARY KEY (`sale_id`,`language_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
		//create sale store table
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "sale_to_store`");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "sale_to_store` (`sale_id` int(11) NOT NULL, `store_id` int(11) NOT NULL, PRIMARY KEY (`sale_id`, `store_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci");
    }
	...
	public function uninstall() { 
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE `query` LIKE 'sale_id=%'");
    }
}
?>